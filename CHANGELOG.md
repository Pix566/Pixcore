...
- Tooltip 规则增强：支持 `combine: all|first`、颜色与粗体/斜体等样式、`translate`、`keybind`、`component-json` 组件。
...
- 盔甲外观：通过 `IClientItemExtensions.getArmorTexture` 替换原版盔甲贴图，支持 inner/outer、head/chest/legs/feet 按槽位贴图与 color 染色。
- 配置逐条校验：非 Map 条目或缺少必填字段时跳过并输出定位日志。
- HUD 性能优化：使用 ID 索引 Map 快速移除/更新条目。
- 拾取 HUD 设置界面实时位置预览。
- 客户端规则查看器：在设置界面查看已加载图标/盔甲/Tooltip 规则，并支持一键禁用/启用全部规则。
- 新增 `zh_cn.json` 本地化。
